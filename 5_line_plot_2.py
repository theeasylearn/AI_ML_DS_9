...
df = sensex.history(period="60d")

# 1. Grab the last 30 rows and immediately reset the index
sensex_30_days_close = df[['Close']].tail(30).reset_index()
...
sensex_30_days_close['Date'] = sensex_30_days_close['Date'].dt.strftime('%m-%d')

sns.lineplot(x='Date', y='Close', data=nifty_30_days_close)
sns.lineplot(x='Date', y='Close', data=sensex_30_days_close)
plt.xlabel("Dates")
plt.ylabel("Close")
plt.title("NIFTY-SENSEX  last 30 days performance")
